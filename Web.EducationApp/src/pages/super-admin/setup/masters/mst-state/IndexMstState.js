 ...
 * Index Page State Master Details
 * --------------------------------------------------------------------------
 */
...
const IndexMstState = (props) => {
    ...
    const funcHandelPDFExport = async () => {
        if (props.defaultDynamicAPIResponse.DataList.length > 0) {
            let filename = "StateMasterData" + format(new Date(), '-ddMMyyyykkmmss'), title = "State Master Data (" + format(new Date(), 'dd/MM/yyyy kk:mm:ss') + ")";
    ...
    const funcHandelCSVExport = async () => {
        if (props.defaultDynamicAPIResponse.DataList.length > 0) {
            let filename = "StateMasterData" + format(new Date(), '-ddMMyyyykkmmss');
