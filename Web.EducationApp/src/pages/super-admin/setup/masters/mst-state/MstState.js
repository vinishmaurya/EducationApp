 ...
 * Root Page State Details
 * --------------------------------------------------------------------------
 */
import IndexMstState from "../../../../../pages/super-admin/setup/masters/mst-state/IndexMstState";
import AddEditMstState from "../../../../../pages/super-admin/setup/masters/mst-state/AddEditMstState";
...
const MstState = (props) => {
    ...
    useEffect(() => {
        if (MyComponent == "IndexMstState") {
            fetchParentDefaultData(StateId, RowPerPage, CurrentPage, SearchBy, SearchValue);
        }
        else if (MyComponent == "AddEditMstState") {
            fetchParentDefaultData(StateId, RowPerPage, CurrentPage, SearchBy, SearchValue);
        }
    }, []);
    ...
    const fetchParentDefaultData = async (parmStateId, parmRowPerPage, parmCurrentPage, parmSearchBy, parmSearchValue) => {
        setRowPerPage(parmRowPerPage);
        setCurrentPage(parmCurrentPage);
        setSearchBy(parmSearchBy);
        setSearchValue(parmSearchValue);
        setStateId(parmStateId);
        ...
        let GetStateDetailsUri = APIConfig.Admin.State.GetStateDetailsUri;
        GetStateDetailsUri = GetStateDetailsUri
            .replace('<StateId>', parmStateId)
    ...
    async function funcDeleteRecord(rowData) {
        ...
        let DeleteStatesDetailsUri = APIConfig.Admin.State.DeleteStatesDetailsUri;
        DeleteStatesDetailsUri = DeleteStatesDetailsUri
            .replace('<StateId>', id)
            .replace('<DeletedBy>', Cookie.loggedInUserId)

        instance({
            'method': 'DELETE',
            'url': DeleteStatesDetailsUri
        }).then((response) => {
            setHasAPIError(false);
            //debugger;
            //console.log(response.data);
                    ...
                    if (MyComponent == "IndexMstState") {
                        ...
                        return (
                            <>
                                ...
                                <AddEditMstState pageTitle={data.innerComponentName} dataRow={DataRow} funcBackToIndex={loadComponent} />
                            </>
                        );
                    }
                }
