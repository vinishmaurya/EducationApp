 ...
 * Root Page Country Details
 * --------------------------------------------------------------------------
 */
import IndexMstCountry from "../../../../../pages/super-admin/setup/masters/mst-country/IndexMstCountry";
import AddEditMstCountry from "../../../../../pages/super-admin/setup/masters/mst-country/AddEditMstCountry";
...
const MstCountry = (props) => {
    ...
    useEffect(() => {
        if (MyComponent == "IndexMstCountry") {
            fetchParentDefaultData(CountryId, RowPerPage, CurrentPage, SearchBy, SearchValue);
        }
        else if (MyComponent == "AddEditMstCountry") {
            fetchParentDefaultData(CountryId, RowPerPage, CurrentPage, SearchBy, SearchValue);
        }
    }, []);
    ...
    const fetchParentDefaultData = async (parmCountryId, parmRowPerPage, parmCurrentPage, parmSearchBy, parmSearchValue) => {
        setRowPerPage(parmRowPerPage);
        setCurrentPage(parmCurrentPage);
        setSearchBy(parmSearchBy);
        setSearchValue(parmSearchValue);
        setCountryId(parmCountryId);
        ...
        let GetCountryDetailsUri = APIConfig.Admin.Country.GetCountryDetailsUri;
        GetCountryDetailsUri = GetCountryDetailsUri
            .replace('<CountryId>', parmCountryId)
    ...
    async function funcDeleteRecord(rowData) {
        ...
        let DeleteCountrysDetailsUri = APIConfig.Admin.Country.DeleteCountrysDetailsUri;
        DeleteCountrysDetailsUri = DeleteCountrysDetailsUri
            .replace('<CountryId>', id)
            .replace('<DeletedBy>', Cookie.loggedInUserId)

        instance({
            'method': 'DELETE',
            'url': DeleteCountrysDetailsUri
        }).then((response) => {
            setHasAPIError(false);
            //debugger;
            //console.log(response.data);
                    ...
                    if (MyComponent == "IndexMstCountry") {
                        ...
                        return (
                            <>
                                ...
                                <AddEditMstCountry pageTitle={data.innerComponentName} dataRow={DataRow} funcBackToIndex={loadComponent} />
                            </>
                        );
                    }
                }
