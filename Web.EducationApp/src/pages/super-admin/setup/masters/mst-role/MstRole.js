...
const MstRole = (props) => {
    ...
    useEffect(() => {
        if (MyComponent == "IndexMstRole") {
            fetchParentDefaultData(RoleId, RowPerPage, CurrentPage, SearchBy, SearchValue);
        }
    ...
    const fetchParentDefaultData = async (parmRoleId, parmRowPerPage, parmCurrentPage, parmSearchBy, parmSearchValue) => {
        //debugger
        setRowPerPage(parmRowPerPage);
        setCurrentPage(parmCurrentPage);
        setSearchBy(parmSearchBy);
        setSearchValue(parmSearchValue);
        setRoleId(parmRoleId);
        ...
        let GetRoleDetailsUri = APIConfig.Admin.Role.GetRoleDetailsUri;
        GetRoleDetailsUri = GetRoleDetailsUri
            .replace('<RoleId>', parmRoleId)
        ...
        instance({
            'method': 'GET',
            'url': GetRoleDetailsUri
        }).then((response) => {
            //console.log(GetRoleDetailsUri);
    ...
    async function funcDeleteRecord(rowData) {
        ...
        let DeleteRolesDetailsUri = APIConfig.Admin.Role.DeleteRolesDetailsUri;
        DeleteRolesDetailsUri = DeleteRolesDetailsUri
            .replace('<RoleId>', id)
            .replace('<DeletedBy>', Cookie.loggedInUserId)

        instance({
            'method': 'DELETE',
            'url': DeleteRolesDetailsUri
        }).then((response) => {
            setHasAPIError(false);
            //debugger;
                    ...
                    if (MyComponent == "IndexMstRole") {
                        ...
                        return (
                            <>
                                ...
                                <AddEditMstRole pageTitle={data.innerComponentName} dataRow={DataRow} funcBackToIndex={loadComponent} />
                            </>
                        );
                    }
                }
