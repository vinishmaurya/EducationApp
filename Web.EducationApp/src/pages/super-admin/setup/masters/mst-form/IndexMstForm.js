 ...
 * Index Page Form Master Details
 * --------------------------------------------------------------------------
 */
...
const IndexMstForm = (props) => {
    ...
    const funcHandelPDFExport = async () => {
        if (props.defaultDynamicAPIResponse.DataList.length > 0) {
            let filename = "FormMasterData" + format(new Date(), '-ddMMyyyykkmmss'), title = "Form Master Data (" + format(new Date(), 'dd/MM/yyyy kk:mm:ss') + ")";
    ...
    const funcHandelCSVExport = async () => {
        if (props.defaultDynamicAPIResponse.DataList.length > 0) {
            let filename = "FormMasterData" + format(new Date(), '-ddMMyyyykkmmss');
